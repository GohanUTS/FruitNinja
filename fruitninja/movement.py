...
class MovementNode(Node):
    def __init__(self):
        ...
        # Declare robot_ip parameter (override via CLI or launch file)
        ...
        self.get_logger().info(f'Targeting real robot at IP: {self._robot_ip}')
    ...
    def move_to(self, positions: list, label: str) -> bool:
        ...
        if not self._client.wait_for_server(timeout_sec=10.0):
            self.get_logger().error(
                ...
                'is the UR driver and MoveIt running?'
            )
            return False
        ...
        # Slower scaling for real robot safety
        ...
        if result.error_code.val == MoveItErrorCodes.SUCCESS:
            ...
            self.get_logger().error(
                f'[FAIL] {label} — error code: {result.error_code.val}'
            )
            return False
...
def main(args=None):
    ...
    print('>>> Make sure External Control URCap is running on Polyscope! <<<')
    ...
    for positions, label in steps:
        success = node.move_to(positions, label)
        if not success:
            print(f'[ABORT] Failed at step: {label}')
            break
        time.sleep(0.5)
